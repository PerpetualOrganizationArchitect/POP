...
contract PaymasterHubInvariantsTest is Test {
     ...
     */
    function testInvariant_AtomicBudgetEnforcement(uint64 cap, uint8 numOps, uint256 seed) public {
     ...
     */
    function testInvariant_EpochConsistency(uint32 epochLen, uint256 timeJump1, uint256 timeJump2) public {
     ...
     */
    function testInvariant_FundSeparation(uint256 depositAmount, uint256 bountyAmount, uint256 withdrawAmount) public {
        ...
        assertEq(entryPoint.balanceOf(address(hub)), initialEPBalance + depositAmount, "EntryPoint deposit mismatch");

        // Fund bounty
        vm.prank(admin);
        hub.fundBounty{value: bountyAmount}();

        assertEq(address(hub).balance, initialContractBalance + bountyAmount, "Bounty fund mismatch");
    ...
    function testInvariant_DeterministicValidation() public {
        ...
        // First validation
        vm.prank(address(entryPoint));
        (bytes memory context1, uint256 validationData1) = hub.validatePaymasterUserOp(userOp, userOpHash, maxCost);

        // Reset state by not executing postOp

        // Second validation with same inputs
        vm.prank(address(entryPoint));
        (bytes memory context2, uint256 validationData2) = hub.validatePaymasterUserOp(userOp, userOpHash, maxCost);
    ...
    function _createUserOp(address sender) internal view returns (PackedUserOperation memory) {
        ...
        bytes memory paymasterAndData =
            abi.encodePacked(address(hub), uint8(1), uint8(0), bytes20(sender), uint32(0), uint64(0));
    ...
    function _createUserOpWithMailbox(address sender) internal view returns (PackedUserOperation memory) {
        ...
        bytes memory paymasterAndData =
            abi.encodePacked(address(hub), uint8(1), uint8(0), bytes20(sender), uint32(0), mailboxCommit8);
