...
contract TaskManagerBootstrapTest is Test {
    ...
    function _buildBootstrapTask(
        uint8 projectIndex,
        string memory title,
        uint256 payout
    ) internal pure returns (TaskManager.BootstrapTaskConfig memory) {
    ...
    function test_BootstrapSingleProjectWithTasks() public {
        ...
        bytes memory result = lens.getStorage(address(tm), TaskManagerLens.StorageKey.PROJECT_INFO, abi.encode(projectIds[0]));
        (uint256 cap, uint256 spent, bool isManager) = abi.decode(result, (uint256, uint256, bool));
        ...
        result = lens.getStorage(address(tm), TaskManagerLens.StorageKey.TASK_INFO, abi.encode(0));
        (uint256 payout, TaskManager.Status status,,, bool requiresApp) = abi.decode(result, (uint256, TaskManager.Status, address, bytes32, bool));
    ...
    function test_BootstrapMultipleProjects() public {
        ...
        projects[0] = _buildBootstrapProject("Project A", 50 ether, _hatArr(CREATOR_HAT), _hatArr(MEMBER_HAT), _hatArr(PM_HAT), _hatArr(PM_HAT));
        projects[1] = _buildBootstrapProject("Project B", 100 ether, _hatArr(CREATOR_HAT), _hatArr(MEMBER_HAT), _hatArr(PM_HAT), _hatArr(PM_HAT));
        ...
        result = lens.getStorage(address(tm), TaskManagerLens.StorageKey.PROJECT_INFO, abi.encode(projectIds[1]));
    ...
    function test_BootstrapWithRolePermissions() public {
        ...
        // Verify claim
        bytes memory result = lens.getStorage(address(tm), TaskManagerLens.StorageKey.TASK_INFO, abi.encode(0));
        (, TaskManager.Status status, address claimer,,) = abi.decode(result, (uint256, TaskManager.Status, address, bytes32, bool));
    ...
    function test_BootstrapTaskLifecycleAfterBootstrap() public {
        ...
        bytes memory result = lens.getStorage(address(tm), TaskManagerLens.StorageKey.TASK_INFO, abi.encode(0));
        (, TaskManager.Status status,,,) = abi.decode(result, (uint256, TaskManager.Status, address, bytes32, bool));
        assertEq(uint8(status), uint8(TaskManager.Status.COMPLETED), "Task should be completed");
    }
}
