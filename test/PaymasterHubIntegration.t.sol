...
contract PaymasterHubIntegrationTest is Test {
    ...
    function testCompleteFlow_AccountBudget() public {
        ...
        (bytes memory context, uint256 validationData) = hub.validatePaymasterUserOp(
            userOp,
            userOpHash,
            0.1 ether
        );
    ...
    function testCompleteFlow_HatBudget() public {
        ...
        vm.prank(address(entryPoint));
        (bytes memory aliceContext, ) = hub.validatePaymasterUserOp(
            aliceOp,
            aliceOpHash,
            0.5 ether
        );
        ...
        PackedUserOperation memory bobOp = _createUserOpWithHat(
            ...
            bob,
            targetContract,
            abi.encodeWithSignature("doSomethingElse()"),
            CONTRIBUTOR_HAT
        );
        ...
        vm.prank(address(entryPoint));
        (bytes memory bobContext, ) = hub.validatePaymasterUserOp(
            bobOp,
            bobOpHash,
            0.5 ether
        );
    ...
    function testCompleteFlow_MailboxAndBounty() public {
        ...
        vm.prank(address(entryPoint));
        (bytes memory context, ) = hub.validatePaymasterUserOp(
            userOp,
            userOpHash,
            0.5 ether
        );
        ...
        assertGt(
            defaultSender.balance,
            0,
            "DefaultSender should receive bounty"
        );
    }
    ...
    function testCompleteFlow_EpochRolling() public {
        ...
        PackedUserOperation memory op1 = _createUserOp(
            address(account1),
            targetContract,
            abi.encodeWithSignature("operation1()"),
            0
        );
        
        vm.prank(address(entryPoint));
        (bytes memory context1, ) = hub.validatePaymasterUserOp(
            op1,
            keccak256(abi.encode(op1)),
            0.8 ether
        );
        ...
        PackedUserOperation memory op2 = _createUserOp(
            address(account1),
            targetContract,
            abi.encodeWithSignature("operation2()"),
            0
        );
        
        vm.prank(address(entryPoint));
        vm.expectRevert(PaymasterHub.BudgetExceeded.selector);
        hub.validatePaymasterUserOp(
            op2,
            keccak256(abi.encode(op2)),
            0.3 ether
        );
        ...
        vm.prank(address(entryPoint));
        (bytes memory context2, ) = hub.validatePaymasterUserOp(
            op2,
            keccak256(abi.encode(op2)),
            0.9 ether
        );
    ...
    function testCompleteFlow_BatchExecution() public {
        ...
        bytes memory batchCallData = abi.encodeWithSelector(
            bytes4(keccak256("executeBatch(address[],bytes[])")),
            targets,
            data
        );
    ...
    function testCompleteFlow_ConcurrentBundlers() public {
        ...
        // Create operations
        PackedUserOperation memory aliceOp = _createUserOp(
            address(account1),
            targetContract,
            abi.encodeWithSignature("aliceAction()"),
            0
        );
        
        PackedUserOperation memory bobOp = _createUserOp(
            address(account2),
            targetContract,
            abi.encodeWithSignature("bobAction()"),
            0
        );
