...
contract MockHats is IHats {
    ...
    function viewHat(uint256) external pure returns (
...
contract PaymasterHubTest is Test {
    ...
    function testValidatePaymasterUserOp_RevertOnDeniedRule() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(PaymasterHub.RuleDenied.selector, address(0x123), bytes4(keccak256("execute(address,uint256,bytes)"))));
        hub.validatePaymasterUserOp(userOp, userOpHash, 0.1 ether);
    }
