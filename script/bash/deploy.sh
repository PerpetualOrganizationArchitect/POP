...
#
# Usage:
#   ./script/bash/deploy.sh                          # Full deployment (all 4 steps)
#   ./script/bash/deploy.sh --step 1                 # Deploy home chain only
#   ./script/bash/deploy.sh --step 2                 # Deploy all satellites
#   ./script/bash/deploy.sh --step 2 --satellite 1   # Deploy Optimism satellite only
#   ./script/bash/deploy.sh --step 3                 # Register satellites + transfer ownership
#   ./script/bash/deploy.sh --step 4                 # Verify deployment
#   ./script/bash/deploy.sh --step summary           # Print deployed addresses
#   ./script/bash/deploy.sh --verify                 # Enable Etherscan verification
#   ./script/bash/deploy.sh --dry-run                # Simulate without broadcasting
#   ./script/bash/deploy.sh --yes                    # Skip confirmation prompt
#############################################################################
...
usage() {
    cat <<'USAGE'
Usage: ./script/bash/deploy.sh [OPTIONS]
...
Examples:
  ./script/bash/deploy.sh                          Full deployment
  ./script/bash/deploy.sh --step 2 --satellite 1   Deploy Optimism satellite only
  ./script/bash/deploy.sh --verify --yes           Full deploy with verification, no prompt
  ./script/bash/deploy.sh --step summary           Print all deployed addresses
USAGE
}
...
cleanup() {
    ...
    if [ $exit_code -ne 0 ] && [ -n "${CURRENT_STEP:-}" ]; then
        ...
        if [ "${CURRENT_STEP_NUM:-}" = "2" ] && [ -n "${CURRENT_SAT_INDEX:-}" ]; then
            error "  ./script/bash/deploy.sh --step 2 --satellite $CURRENT_SAT_INDEX"
        elif [ -n "${CURRENT_STEP_NUM:-}" ]; then
            error "  ./script/bash/deploy.sh --step $CURRENT_STEP_NUM"
        fi
    fi
}
trap cleanup EXIT
...
step2_deploy_satellites() {
    ...
    if [ ! -f "$STATE_FILE" ]; then
        ...
        error "Run step 1 first: ./script/bash/deploy.sh --step 1"
        exit 1
    fi
...
step3_register_and_transfer() {
    ...
    for i in "${!SAT_DOMAINS[@]}"; do
        ...
        if [ ! -f "$sat_file" ]; then
            ...
            error "Run step 2 for ${SAT_NAMES[$i]} first: ./script/bash/deploy.sh --step 2 --satellite $i"
            exit 1
        fi
    done
