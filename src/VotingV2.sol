...
contract VotingV2 is Initializable {
     ...
     */
    function createProposal(uint256 proposalId, string memory metadata, string memory _requiredRole)
        external
        onlyOwner
    {
    ...
    function vote(uint256 proposalId, bool voteValue) external {
        ...
        if (bytes(role).length > 0) {
            require(IMembershipV2(membershipContract).hasRole(msg.sender, role), "Member doesn't have required role");
        }
